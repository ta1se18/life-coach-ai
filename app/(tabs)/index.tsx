  ...
  const generatedPlan = useMemo(() => {
    ...
    if (text.includes('起業') || text.includes('事業')) {
      return {
        ...
        today: ['競合サービスを1つ分析する', '事業アイデアを10分書く', '営業・マーケについて学ぶ'],
      };
    }
    ...
    if (text.includes('筋トレ') || text.includes('健康')) {
      return {
        analysis: 'あなたは「自信」「自己管理」「理想の自分」を求めている可能性があります。',
        short: '睡眠・食事・運動を整える。',
        middle: '継続できる身体習慣を作る。',
        long: '健康で自信のある状態を維持する。',
        today: ['筋トレ20分', '睡眠7時間', '高タンパク食事'],
      };
    }
    ...
    <ScrollView style={styles.container}>
        ...
        <View style={styles.card}>
          ...
          {remaining && (
            <View style={styles.lifeBox}>
              ...
              <Text style={styles.note}>時間は有限。だから今日を決める。</Text>
            </View>
          )}

          <Pressable style={styles.button} onPress={() => setPhase('hearing')}>
      ...
      {phase === 'hearing' && (
        <View style={styles.card}>
          ...
          <TextInput style={styles.textarea} placeholder="夢・なりたい姿" placeholderTextColor="#999" value={dream} onChangeText={setDream} multiline />
          ...
          <View style={styles.chatBox}>
            ...
            <Pressable style={styles.button} onPress={() => setDeepStep(deepStep + 1)}>
              <Text style={styles.buttonText}>もう少し深掘りする</Text>
            </Pressable>

            <Pressable style={styles.subButton} onPress={() => setPhase('plan')}>
      ...
      {phase === 'plan' && (
        <View style={styles.card}>
          ...
          <Text style={styles.analysis}>{generatedPlan.analysis}</Text>

          <Text style={styles.sectionTitle}>短期計画</Text>
          <TextInput style={styles.input} placeholder="例：2週間、1ヶ月" placeholderTextColor="#999" value={shortTerm} onChangeText={setShortTerm} />
          <Text style={styles.resultText}>{generatedPlan.short}</Text>

          <Text style={styles.sectionTitle}>中期計画</Text>
          <TextInput style={styles.input} placeholder="例：6ヶ月、1年" placeholderTextColor="#999" value={middleTerm} onChangeText={setMiddleTerm} />
          <Text style={styles.resultText}>{generatedPlan.middle}</Text>

          <Text style={styles.sectionTitle}>長期計画</Text>
          <TextInput style={styles.input} placeholder="例：5年、10年" placeholderTextColor="#999" value={longTerm} onChangeText={setLongTerm} />
          <Text style={styles.resultText}>{generatedPlan.long}</Text>
          ...
          <Pressable style={styles.subButton} onPress={() => setPhase('hearing')}>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: '#fff', borderRadius: 12, padding: 14, fontSize: 16, marginBottom: 12, borderWidth: 1, borderColor: '#ddd' },
  textarea: { backgroundColor: '#fff', borderRadius: 12, padding: 14, fontSize: 16, marginBottom: 12, minHeight: 90, borderWidth: 1, borderColor: '#ddd' },
  row: { flexDirection: 'row', gap: 10, marginBottom: 16 },
  choice: { flex: 1, padding: 14, borderRadius: 12, borderWidth: 1, borderColor: '#ddd', alignItems: 'center', backgroundColor: '#fff' },
  ...
  note: { color: '#aaa', marginTop: 12, fontSize: 13 },
